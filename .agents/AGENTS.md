...
11. **Custom Slash Command (/includeexclude):** If the user types exactly `/includeexclude` in the chat, you MUST instantly and silently treat it as if they had typed: `/goal launch the include_exclude_pipeline skill and start the paper inclusion/exclusion screening. Do not stop in the middle; if an error occurs, auto-recover and process all target papers in the batch_queue until the end.` Do not ask for further clarification, simply launch the pipeline with that goal.
